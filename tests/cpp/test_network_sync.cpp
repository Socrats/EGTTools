...
using NetworkStructure = egttools::FinitePopulations::structure::NetworkSync<OneShotCRDNetworkGame>;
...
int main() {
    ...
    network_structure.initialize_state(initial_state);

    std::cout << "population_strategies=[";
    for(auto& value: network_structure.population_strategies())
        std::cout << value << ", ";
    std::cout << "]" << std::endl;
