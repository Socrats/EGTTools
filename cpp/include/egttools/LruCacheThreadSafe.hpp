...
#pragma once
#ifndef EGTTOOLS_LRUCACHETHREADSAFE_HPP
#define EGTTOOLS_LRUCACHETHREADSAFE_HPP
...
namespace egttools::Utils {
    ...
    public:
         ...
         */
        explicit LRUCacheThreadSafe(size_t size);

        ~LRUCacheThreadSafe() { clear(); }
    ...
    template<class TKey, class TValue>
    LRUCacheThreadSafe<TKey, TValue>::LRUCache(size_t size) : max_size_(size) {
    ...
    template<class TKey, class TValue>
    void LRUCacheThreadSafe<TKey, TValue>::insert(const TKey &key, const TValue &value) {
    ...
    template<class TKey, class TValue>
    TValue& LRUCacheThreadSafe<TKey, TValue>::get(const TKey &key) {
        assert(exists(key));
        auto it = cache_map_.find(key);
    ...
    template<class TKey, class TValue>
    bool LRUCacheThreadSafe<TKey, TValue>::get(const TKey &key, TValue &value) {
        auto it = cache_map_.find(key);
    ...
    template<class TKey, class TValue>
    void LRUCacheThreadSafe<TKey, TValue>::erase(const TKey &key) {
        assert(exists(key));
        auto it = cache_map_.find(key);
    ...
    template<class TKey, class TValue>
    inline bool LRUCacheThreadSafe<TKey, TValue>::exists(const TKey &key) {
        return (cache_map_.count(key) > 0);
    }

    template<class TKey, class TValue>
    void LRUCacheThreadSafe<TKey, TValue>::clear() {
        item_list_.clear();
        cache_map_.clear();
    }

    template<class TKey, class TValue>
    inline void LRUCacheThreadSafe<TKey, TValue>::clean_extra_space_() {
    ...
    template<class TKey, class TValue>
    size_t LRUCacheThreadSafe<TKey, TValue>::max_size() const {
        return max_size_;
    }

    template<class TKey, class TValue>
    size_t LRUCacheThreadSafe<TKey, TValue>::current_size() const {
        return cache_map_.size();
    }

    template<class TKey, class TValue>
    void LRUCacheThreadSafe<TKey, TValue>::set_max_size(size_t max_size) {
        max_size_ = max_size;
    }
}
...
#endif //EGTTOOLS_LRUCACHETHREADSAFE_HPP
