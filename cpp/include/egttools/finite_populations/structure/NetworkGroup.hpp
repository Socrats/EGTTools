...
namespace egttools::FinitePopulations::structure {
    /**
     * @brief Asynchronous network structure for N-player group games.
     *
     * Each node plays k+1 games per generation, where k is its degree:
     * one game as focal player in its own neighbourhood, and one game as a
     * participant in each neighbour's neighbourhood. Total fitness is the
     * accumulated payoff across all k+1 games.
     *
     * The update rule and topology conventions are identical to Network<>.
     */
    ...
    double NetworkGroup<GameType, CacheType>::calculate_fitness(int index) {
        ...
        for (int nb : network_[index]) {
            fitness += calculate_game_payoff(nb);
        }
        return fitness;
    }
    ...
    double NetworkGroup<GameType, CacheType>::calculate_game_payoff(int index) {
        ...
        if (!cache_.exists(key)) {
            double payoff = game_.calculate_fitness(population_[index], neighbourhood_state_);
    ...
    template<class GameType, class CacheType>
    int NetworkGroup<GameType, CacheType>::population_size() { return population_size_; }

    template<class GameType, class CacheType>
    int NetworkGroup<GameType, CacheType>::nb_strategies() { return nb_strategies_; }

    template<class GameType, class CacheType>
    const AdjacencyList &NetworkGroup<GameType, CacheType>::network() { return network_; }

    template<class GameType, class CacheType>
    std::vector<int> NetworkGroup<GameType, CacheType>::population_strategies() const { return population_; }

    template<class GameType, class CacheType>
    VectorXui &NetworkGroup<GameType, CacheType>::mean_population_state() { return mean_population_state_; }

    template<class GameType, class CacheType>
    GameType &NetworkGroup<GameType, CacheType>::game() { return game_; }
