...
FinitePopulations::MLS<FinitePopulations::GarciaGroup>::MLS(size_t generations,
...
double FinitePopulations::MLS<FinitePopulations::GarciaGroup>::fixationProbability(
        size_t invader, size_t resident, size_t runs,
...
#pragma omp parallel for shared(group_strategies) reduction(+:r2m, r2r)
    ...
    for (size_t i = 0; i < runs; ++i) {
        ...
        FinitePopulations::GarciaGroup group(_nb_strategies, _group_size, w, group_strategies, payoff_in,
                               payoff_out);
        std::vector<FinitePopulations::GarciaGroup> groups(_nb_groups, group);
...
void
FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_update(
        double q, double lambda, double alpha, double kappa, double z,
        std::vector<FinitePopulations::GarciaGroup> &groups,
...
void FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_createMutant(
        size_t invader, size_t resident,
        std::vector<FinitePopulations::GarciaGroup> &groups) {
    auto mutate_group = _uint_rand(_get_rng());
    groups[mutate_group].createMutant(invader, resident);
}

void FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_createRandomMutant(
        size_t invader,
        std::vector<FinitePopulations::GarciaGroup> &groups,
...
void FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_updateFullPopulationFrequencies(
        size_t increase, size_t decrease,
        egttools::VectorXui &strategies) {
    ++strategies(increase);
    --strategies(decrease);
}
...
void
FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_reproduce_garcia(
        std::vector<FinitePopulations::GarciaGroup> &groups, VectorXui &strategies,
        const double &lambda,
        const double &alpha) {
    ...
    if (_real_rand(_get_rng()) < lambda) groups[_migrate(parent_group, new_strategy, groups)].totalPayoff(alpha, strategies);
    // No else branch: totalPayoff already updated in _payoffProportionalSelection (B5 fix)
}

size_t
FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_migrate(
        const size_t &parent_group, const size_t &migrating_strategy,
        std::vector<FinitePopulations::GarciaGroup> &groups) {
...
void FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_mutate(
        std::vector<FinitePopulations::GarciaGroup> &groups, VectorXui &strategies) {
...
void
FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_splitGroup(
        size_t parent_group,
        std::vector<FinitePopulations::GarciaGroup> &groups,
        VectorXui &strategies) {
...
size_t
FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_payoffProportionalSelection(
        const double &alpha, std::vector<FinitePopulations::GarciaGroup> &groups,
        VectorXui &strategies) {
...
size_t FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_sizeProportionalSelection(
        std::vector<FinitePopulations::GarciaGroup> &groups) {
...
bool FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_pseudoStationary(
        std::vector<FinitePopulations::GarciaGroup> &groups) {
...
void FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_setFullHomogeneousState(
        size_t strategy, std::vector<FinitePopulations::GarciaGroup> &groups) {
    for (auto &group: groups)
        group.setPopulationHomogeneous(strategy);
}

size_t FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_current_pop_size(
        std::vector<FinitePopulations::GarciaGroup> &groups) {
    size_t size = 0;
...
void FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_setState(
        std::vector<FinitePopulations::GarciaGroup> &groups, std::vector<size_t> &container) {
    // Then we shuffle it randomly the container
...
void
FinitePopulations::MLS<FinitePopulations::GarciaGroup>::_resolve_conflict(
