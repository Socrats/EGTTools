...
if [[ $(uname) == "Darwin" ]]; then
  if [[ "$CIBW_BUILD" == *-macosx_arm64 || "$CIBW_BUILD" == *-macosx_universal2:arm64 ]]; then
    ...
    export EGTTOOLS_EXTRA_CMAKE_ARGS="-DLIBOMP_DIR='$(brew --prefix libomp)' -DMACOSX_DEPLOYMENT_TARGET=12.0"
    echo "MACOSX_DEPLOYMENT_TARGET=12.0" >>"$GITHUB_ENV"

  else
    brew reinstall libomp
    export MACOSX_DEPLOYMENT_TARGET=10.15
    export EGTTOOLS_EXTRA_CMAKE_ARGS="-DLIBOMP_DIR='$(brew --prefix libomp)' -DMACOSX_DEPLOYMENT_TARGET=10.15"
